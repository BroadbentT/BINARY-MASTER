...
def dispMenu():
   print('\u2554' + ('\u2550')*37 + '\u2566' + ('\u2550')*36 + '\u2566' + ('\u2550')*28 + '\u2566' + ('\u2550')*18 + '\u2566' + ('\u2550')*38 + '\u2566' + ('\u2550')*76 + '\u2566' + ('\u2550')*76 + '\u2557')
   print('\u2551' + " REGISTER SET TWO " + (" ")*19 + '\u2551' + " REGISTER SET ONE " + (" ")*18 + '\u2551' + " FILE INFORMATION " + (" ")*10 + '\u2551' + " CHECKSEC DATA    " + '\u2551' + " OFFSET              FUNCTIONS        " + '\u2551' + " GADGETS INFORMATION                                                        " + '\u2551' + " PYTHON EXPLOIT CODE                                                        " + '\u2551' )
   print('\u2560' + ('\u2550')*16 + '\u2566' + ('\u2550')*20 + '\u256C' + ('\u2550')*15 + '\u2566' + ('\u2550')*20 + '\u256C' + ('\u2550')*28 + '\u256C' + ('\u2550')*18 + '\u256C' +  ('\u2550')*38 + '\u2563' +  ('\u2550')*76 + '\u2563'+  ('\u2550')*76 + '\u2563')
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --- -- -- --- -  
   for x in range(0,12):
      ...
      print('\u2551', end=' ')	
      print(colored(GADG[x], colour6), end=' ')
      print('\u2551', end=' ')
      print(colored(CODE[x], colour5, attrs=['bold']), end=' ')
      print('\u2551')		
...
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --- -- -- --- -  
   ...
   print('\u2551' + " ", end='')
   print(colored(GADG[12] + " ", colour6), end='')
   print('\u2551' + " " , end='')
   print(colored(CODE[12] + " " , colour5, attrs=['bold']), end='')
   print('\u2551')    
   return
...
# -------------------------------------------------------------------------------------
   ...
   if selection == '31':
      if DATA[0][:7].upper() == "UNKNOWN":
         ...
         cutLine("============================================================", gadgFile)    
         with open(gadgFile,"r") as gadgets:
            for x in range(0, maxDispl):
               GADG[x] = gadgets.readline()
               GADG[x] = spacePadding(GADG[x], COL4)
      prompt()
...
# UPDATE DISPLAY CODE
         os.system("cp " + localDir + "/exploit.py temp.tmp")
         parsFile("temp.tmp")
         cutLine("#", "temp.tmp")
         cutLine("0x0000000000000000","temp.tmp")        
         with open("temp.tmp", "r") as code:
            for x in range(0, maxDispl):
               CODE[x] = code.readline()
               CODE[x] = spacePadding(CODE[x], COL4)
# EXPLOIT
         os.chdir(localDir)
